...
(function () {
            ...
            case "object":
                if (mixed_val == null) return "N;"; else if (mixed_val instanceof Array) {
                    ...
                    for (var i = 0; i < mixed_val.length; i++) {
                        ...
                        if (ser) map.push(raiderspirit_serialize(idxobj.idx) + ser);
                    }
                    ...
                    for (var prop in mixed_val) {
                        ...
                        if (ser) props.push(raiderspirit_serialize(prop) + ser);
                    }
    ...
    window.raiderspirit_get_date_difference = function (dt1) {
        ...
        } else var a2 = raiderspirit_parse_date(dt2);
    ...
    window.raiderspirit_color_picker = function () {
        ...
        tbl += '</tr></thead><tbody>' + '<tr class="height_60">' + '<td colspan="8" id="' + id + '_colorPreview" class="colorpicker_extra_style">' + '<input class="colorpicker_input_extra_style" maxlength="7" />' + '<a href="#" id="' + id + '_moreColors" class="iColorPicker_moreColors"></a>' + '</td>' + '<td colspan="8" id="' + id + '_colorOriginal" class="colorpicker_extra_style">' + '<input class="colorpicker_input_extra_style" readonly="readonly" />' + '</td>' + '</tr></tbody></table>';
    ...
    window.raiderspirit_del_cookie = function (name, path, domain) {
        ...
        if (raiderspirit_get_cookie(name)) document.cookie = name + '=' + ((path) ? ';path=' + path : '') + ((domain) ? ';domain=' + domain : '') + ';expires=Thu, 01-Jan-1970 00:00:01 GMT';
    };
    ...
    window.raiderspirit_debug_object = function (obj) {
        ...
        if (obj !== null && (typeof (obj) == 'object' || typeof (obj) == 'array')) {
            for (var prop in obj) {
                ...
                if (level < recursive && (typeof (obj[prop]) == 'object' || typeof (obj[prop]) == 'array') && obj[prop] != obj) dispStr += addStr + prop + '=' + raiderspirit_debug_object(obj[prop], recursive, showMethods, level + 1); else {
                    try {
                        curStr = "" + obj[prop];
                    } catch (e) {
                        curStr = "--- Not evaluate ---";
    ...
    window.dco === undefined && (window.dco = function (s, r) {
        console.log(raiderspirit_debug_object(s, r));
    });
    window.dal === undefined && (window.dal = function (s) {
        if (RAIDERSPIRIT_STORAGE['user_logged_in']) alert(s);
    });
    window.dao === undefined && (window.dao = function (s, r) {
        if (RAIDERSPIRIT_STORAGE['user_logged_in']) alert(raiderspirit_debug_object(s, r));
    });
    window.ddl === undefined && (window.ddl = function (s, c) {
        raiderspirit_debug_log(s, c);
    });
    window.ddo === undefined && (window.ddo = function (s, r, c) {
        raiderspirit_debug_log(raiderspirit_debug_object(s, r), c);
    });
})();
jQuery(document).ready(function () {
    ...
    function raiderspirit_ready_actions() {
        ...
        if (document.documentElement.className.indexOf(RAIDERSPIRIT_STORAGE['site_scheme']) == -1) document.documentElement.className += ' ' + RAIDERSPIRIT_STORAGE['site_scheme'];
        ...
        jQuery('.menu_mobile,.sc_layouts_menu_dir_vertical').on('click', 'li a, li a .open_child_menu', function (e) {
            ...
            if (!jQuery(this).hasClass('open_child_menu') && jQuery(this).parents('.menu_mobile').length > 0 && raiderspirit_is_local_link($a.attr('href'))) jQuery('.menu_mobile_close').trigger('click');
        ...
        jQuery('.nav-links-more a').on('click', function (e) {
            ...
            if (panel.length == 0) {
                jQuery.get(location.href, {paged: page + 1}).done(function (response) {
                    ...
                    raiderspirit_loadmore_add_items(jQuery('.content .posts_container').eq(0), jQuery(response).find('.content .posts_container > article,' + '.content .posts_container > div[class*="column-"],' + '.content .posts_container > .masonry_item'));
                });
            } else {
            ...
            function raiderspirit_loadmore_add_items(container, items) {
                ...
                if (page + 1 >= max_page) more.parent().hide(); else RAIDERSPIRIT_STORAGE['load_more_link_busy'] = false;
                jQuery(window).trigger('scroll');
    ...
    function raiderspirit_stretch_sidemenu() {
        ...
        var toc_items_height = jQuery(window).height() - raiderspirit_fixed_rows_height(true, false) - jQuery('.menu_side_wrap .sc_layouts_logo').outerHeight() - toc_items.length;
    ...
    jQuery(document).on('action.toc_menu_item_active', function () {
        ...
        var th = toc_items.eq(0).height(), toc_menu_pos = parseFloat(toc_menu.css('top')),
            ...
            menu_side_height = jQuery(window).height() - raiderspirit_fixed_rows_height(true, false) - jQuery('.menu_side_wrap .sc_layouts_logo').outerHeight() - jQuery('.menu_side_wrap .sc_layouts_logo + .toc_menu_item').outerHeight();
    ...
    function raiderspirit_check_layout() {
        ...
        if (RAIDERSPIRIT_STORAGE['mobile_device'] || raiderspirit_browser_is_mobile()) jQuery('body').addClass('mobile_device');
    }
        ...
        if (sb.length > 0) {
            if (content.css('float') == 'none') {
                ...
                if (sb_height < content_height && scroll_offset + top_panel_fixed_height > content_top) {
                    ...
                    if (scroll_dir == 0) scroll_dir = RAIDERSPIRIT_STORAGE['scroll_dir_last']; else scroll_dir = scroll_dir > 0 ? 1 : -1;
                    ...
                    if (sb_init.position != 'undefined') {
                        ...
                        if (sb.hasClass('right')) sb_init.right = sb_init.position == 'fixed' || jQuery('body').hasClass('body_style_fullwide') ? pos : 0; else sb_init.left = sb_init.position == 'fixed' || jQuery('body').hasClass('body_style_fullwide') ? pos : 0;
        ...
        if (RAIDERSPIRIT_STORAGE['use_mediaelements'] && cont.find('audio:not(.inited),video:not(.inited)').length > 0) {
            if (window.mejs) {
                ...
                cont.find('audio:not(.inited),video:not(.inited)').each(function () {
                    ...
                    if (jQuery(this).addClass('inited').parents('.mejs-mediaelement').length == 0 && jQuery(this).parents('.wp-block-video').length == 0 && !jQuery(this).hasClass('wp-block-cover__video-background') && (RAIDERSPIRIT_STORAGE['init_all_mediaelements'] || (!jQuery(this).hasClass('wp-audio-shortcode') && !jQuery(this).hasClass('wp-video-shortcode') && !jQuery(this).parent().hasClass('wp-playlist') && jQuery(this).parents('.elementor-background-video-container').length == 0))) {
    ...
    function raiderspirit_tabs_ajax_content_loader(panel, page, oldPanel) {
        ...
        } else panel.find('> *').addClass('raiderspirit_tab_content_remove');
    ...
    function raiderspirit_comments_validate(form) {
        ...
        var comments_args = {
            error_message_text: RAIDERSPIRIT_STORAGE['strings']['error_global'],
            error_message_show: true,
            error_message_time: 4000,
            error_message_class: 'raiderspirit_messagebox raiderspirit_messagebox_style_error',
            ...
            rules: [{
                ...
                max_length: {
                    value: RAIDERSPIRIT_STORAGE['comment_maxlength'],
                    message: RAIDERSPIRIT_STORAGE['strings']['text_long']
                }
            }]
        };
        ...
        if (form.find('.comments_email input[aria-required="true"]').length > 0) {
            comments_args.rules.push({
                ...
                mask: {
                    value: RAIDERSPIRIT_STORAGE['email_mask'],
                    message: RAIDERSPIRIT_STORAGE['strings']['email_not_valid']
                }
            });
        }
...
jQuery(document).on('action.init_hidden_elements', function (e, cont) {
    ...
    if (RAIDERSPIRIT_STORAGE['button_hover'] && RAIDERSPIRIT_STORAGE['button_hover'] != 'default') {
        jQuery('button:not(.search_submit):not([class*="sc_button_hover_"]),\ .theme_button:not([class*="sc_button_hover_"]),\ .sc_button:not([class*="sc_button_simple"]):not([class*="sc_button_hover_"]),\ .sc_form_field button:not([class*="sc_button_hover_"]),\ .post_item .more-link:not([class*="sc_button_hover_"]),\ .trx_addons_hover_content .trx_addons_hover_links a:not([class*="sc_button_hover_"]),\ .raiderspirit_tabs .raiderspirit_tabs_titles li a:not([class*="sc_button_hover_"]),\ .hover_shop_buttons .icons a:not([class*="sc_button_hover_style_"]),\ .mptt-navigation-tabs li a:not([class*="sc_button_hover_style_"]),\ .edd_download_purchase_form .button:not([class*="sc_button_hover_style_"]),\ .edd-submit.button:not([class*="sc_button_hover_style_"]),\ .widget_edd_cart_widget .edd_checkout a:not([class*="sc_button_hover_style_"]),\ .woocommerce #respond input#submit:not([class*="sc_button_hover_"]),\ #btn-buy,\ .woocommerce .button:not([class*="shop_"]):not([class*="view"]):not([class*="sc_button_hover_"]),\ .woocommerce-page .button:not([class*="shop_"]):not([class*="view"]):not([class*="sc_button_hover_"]),\ #buddypress a.button:not([class*="sc_button_hover_"]),\ .post_meta_item.post_categories a,\ .related_wrap .post_cat>a,\ .sc_blogger_default .post_cat>a,\ .minimal-light .esg-loadmore-wrapper .esg-navigationbutton,\ .wp-block-button:not(.is-style-outline) .wp-block-button__link\ ').addClass('sc_button_hover_just_init sc_button_hover_' + RAIDERSPIRIT_STORAGE['button_hover']);
        ...
        jQuery('.sidebar .trx_addons_tabs .trx_addons_tabs_titles li a:not([class*="sc_button_hover_style_"]),\ .raiderspirit_tabs .raiderspirit_tabs_titles li a:not([class*="sc_button_hover_style_"])\ ').addClass('sc_button_hover_just_init sc_button_hover_style_alterbd');
        ...
        jQuery('.sc_button.color_style_link2:not([class*="sc_button_simple"]):not([class*="sc_button_hover_style_"]),\ .widget_tag_cloud a:not([class*="sc_button_hover_style_"]),\ .widget_product_tag_cloud a:not([class*="sc_button_hover_style_"]),\ .vc_tta-color-grey.vc_tta-style-classic .vc_tta-tab > a:not([class*="sc_button_hover_style_"]),\ .woocommerce .widget_price_filter .price_slider_amount .button,\ .add_to_cart_button,\ .button.product_type_grouped,\ .button.product_type_external,\ .single-product div.product .woocommerce-tabs .wc-tabs li a:not([class*="sc_button_hover_style_"])\ ').addClass('sc_button_hover_just_init sc_button_hover_style_link2');
        ...
        jQuery('.mejs-controls button,\ .mfp-close,\ .sc_button_bg_image,\ .hover_shop_buttons a,\ button.pswp__button,\ .woocommerce-orders-table__cell-order-actions .button,\ .sc_layouts_row_type_narrow .sc_button,\ .service_btn_simple,\ .sc_price_item_link:not([class*="sc_button_hover_style_"]),\ .sc_button_border,\ .esg-navigationbutton,\ .woocommerce nav.woocommerce-pagination ul li a,\ .woocommerce-mini-cart__buttons .button.wc-forward:not(.checkout)\ ').removeClass('sc_button_hover_' + RAIDERSPIRIT_STORAGE['button_hover']);
    }
});
(function () {
    "use strict";
...
(function () {
    ...
    jQuery(document).on('action.prepare_stretch_width', function () {
        if (RAIDERSPIRIT_STORAGE['stretch_tabs_area'] > 0) jQuery('.single-product .woocommerce-tabs').wrap('<div class="trx-stretch-width"></div>');
    });
})();